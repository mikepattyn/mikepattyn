...
- Normal app tree: `'apps/<path>/**'`
- Git submodule gitlink only (parent does not see nested file paths): `'apps/<submodule>'` with no `/**` — see kapsalon
...
- Checkout: plain `actions/checkout@v4` unless submodule → token + `submodules: recursive` + `fetch-depth: 0`
